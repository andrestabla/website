/**
 * Genera el .docx de la cotización UPC a partir de sus páginas (content.pages),
 * para que el Word diga exactamente lo mismo que la vista online y el PDF.
 * Respeta la línea gráfica: lockup Algoritmo T en encabezado y portada, navy
 * para títulos, dorado en los antetítulos, tablas con cabecera oscura y las
 * capturas de la plataforma embebidas.
 *
 *   npx tsx scripts/build-upc-docx.ts
 */
...
const OUT = path.join(PUB, 'cotizaciones/upc-2026/Propuesta UPC 2026 - Transformacion Digital (editable).docx')
...
const mark = fs.readFileSync(path.join(PUB, 'assets/algoritmot-mark.svg'))
const markPng = fs.existsSync(path.join(PUB, 'assets/algoritmot-mark.png'))
  ? fs.readFileSync(path.join(PUB, 'assets/algoritmot-mark.png'))
  : null
...
function blockToDocx(b: any): (Paragraph | Table)[] {
  ...
  switch (b.type) {
    ...
    case 'img': {
      const rel = String(b.url || '').replace(/^\//, '')
      const file = path.join(PUB, rel)
      if (fs.existsSync(file)) {
        out.push(new Paragraph({
          children: [new ImageRun({ data: fs.readFileSync(file), transformation: { width: 600, height: Math.round(600 * ratioOf(file)) } } as any)],
...
/* ── Documento ──────────────────────────────────────────────────────────── */
const quote = await db.quote.findUnique({ where: { id: QUOTE_ID } })
if (!quote) throw new Error('No existe la cotización UPC')
const content = quote.content as any
const pages: any[] = content.pages || []
const items: any[] = (quote.pricing as any)?.items || []
...
const brandRun = (size: number) => ([
  ...
  ...(markPng ? [new ImageRun({ data: markPng, transformation: { width: size / 1.6, height: size / 1.6 } } as any)] : [new TextRun({ text: '  T', bold: true, size, color: GOLD, font: 'JetBrains Mono' })]),
])
...
body.push(P(quote.subtitle || '', { size: 22, color: MUTED, after: 400 }))
body.push(new Table({
  width: { size: 100, type: WidthType.PERCENTAGE },
  borders: { top: hairline, bottom: hairline, left: noBorders.left, right: noBorders.right, insideHorizontal: hairline, insideVertical: noBorders.left },
  rows: [
    ['Cliente', quote.clientName],
    ['Duración', '6 meses de ejecución · 18 meses de licencia'],
    ['Inversión', `${money(quote.totalFinal)} COP`],
    ['Validez', `${quote.validDays} días`],
...
body.push(new Paragraph({ text: '', pageBreakBefore: false, spacing: { after: 200 } }))
...
pages.forEach((pg, i) => {
  ...
  if (pg.kicker || pg.title) {
    body.push(new Paragraph({
      children: [
        ...
        ...(pg.kicker ? [new TextRun({ text: pg.kicker.toUpperCase(), size: 17, color: CYAN, font: 'JetBrains Mono' })] : []),
      ],
      spacing: { after: 60 },
    }))
    body.push(new Paragraph({
  ...
  pg.blocks.forEach((b: any) => {
    if (b.type === 'toc') {
      // índice con los títulos y su hoja
      const entries = pages.filter((p, k, all) => {
        if (!p.title || p.tocHidden) return false
        const prev = all.slice(0, k).filter((x) => x.title && !x.tocHidden).pop()
    ...
    blockToDocx(b).forEach((el) => body.push(el))
  })
})
  ...
  sections: [{
    ...
    footers: {
      default: new Footer({
        children: [new Paragraph({
          children: [
            new TextRun({ text: 'Algoritmo T · Transformación Digital UPC', size: 14, color: MUTED, font: 'JetBrains Mono' }),
...
console.log(`OK · ${OUT.split('/').pop()} · ${pages.length} páginas · ${kb} KB`)
await db.$disconnect()
