...
import type { HomeResponsiveViewport, HomeSectionId, HeroContent } from '../admin/context/CMSContext'
...
export function CorporateHome() {
    ...
    // Fixed Hero content as requested by user
    const fixedHeroContent: HeroContent = {
        ...translatedState.hero,
    ...
    // We force hero to be visible and at top for this "fixed" landing
    const isVisibleInViewport = (sectionId: HomeSectionId) => {
        ...
        const sectionVisibility = rawLayout?.sectionVisibility?.[sectionId]
    ...
    const isBlockVisibleInViewport = (sectionId: HomeSectionId, blockId: string) => {
        const sectionBlocks = rawLayout?.blockVisibility?.[sectionId] as Record<string, Partial<Record<HomeResponsiveViewport, boolean>>> | undefined
    ...
    const getBlockStyleStringInViewport = (
        ...
        const raw = (rawLayout?.blockStyleOverrides as any)?.[sectionId]?.[blockId]?.[prop]?.[viewport]
    ...
    const getBlockOrder = <T extends string>(sectionId: 'services' | 'products' | 'frameworks' | 'contact', fallback: readonly T[]) =>
        normalizeBlockOrder((rawLayout?.blockOrder as any)?.[sectionId], fallback)

    const sectionRenderers: Record<HomeSectionId, () => ReactElement> = {
        hero: () => (
            <div id="inicio" data-track-section="home-hero">
                <HeroView 
                    hero={fixedHeroContent} 
                    heroSection={translatedState.homePage.hero} 
                    viewport={viewport} 
                    visibleBlocks={{
                        headline: true,
                        ctas: true,
                        stats: isBlockVisibleInViewport('hero', 'stats')
                    }}
                />
            </div>
        ),
