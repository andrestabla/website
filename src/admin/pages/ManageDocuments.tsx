    ...
    <div>
      ...
      >
        ...
        {isSelected || expanded ? (
          <FolderOpen
            size={14}
            className={node.color ? "" : "text-amber-400"}
        ...
        <div className="hidden group-hover:flex items-center gap-0.5">
          ...
          <button
            ...
            className="p-0.5 rounded hover:bg-zinc-600 text-red-500"
            title="Eliminar"
          >
            <Trash2 size={12} />
          </button>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; class: string }> = {
    ...
    ARCHIVED: {
      label: "Archivado",
      class: "bg-zinc-600/10 text-zinc-500 border-zinc-600/20",
    },
  };
  const s = map[status] ?? {
    label: status,
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${s.class}`}
    >
      {s.label}
    </span>
  );
...
export function ManageDocuments() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (!presignData.ok)
        throw new Error(presignData.error || "Error al autorizar subida");
      ...
      if (!uploadToR2Res.ok)
        throw new Error("Error al subir el archivo directamente a R2");
      ...
      if (!saveData.ok)
        throw new Error(saveData.error || "Error al guardar registro");

      // Step 4: AI
      setUploadStep("ai");
      await fetch(`/api/admin/documents/ai-process?id=${saveData.data.id}`, {
        method: "POST",
      }).catch(() => {});
  ...
  async function handleSaveMetadata() {
    ...
    const res = await fetch(
      `/api/admin/documents/manage?id=${selectedDoc.id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function handleAiProcess() {
    ...
    setAiLoading(true);
    const res = await fetch(
      `/api/admin/documents/ai-process?id=${selectedDoc.id}`,
      { method: "POST" },
    );
    const data = await res.json();
    setAiLoading(false);
  ...
  async function handleDeleteDoc(doc: DocumentItem) {
    if (
      !confirm(
        `¿${doc.status === "ARCHIVED" ? "Eliminar permanentemente" : "Archivar"} "${doc.title}"?`,
      )
    )
      return;
    const res = await fetch(`/api/admin/documents/manage?id=${doc.id}`, {
  ...
  async function handleSendComment() {
    ...
    const res = await fetch(
      `/api/admin/documents/comments?id=${selectedDoc.id}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleRequestReview() {
    ...
    const res = await fetch(
      `/api/admin/documents/review?id=${selectedDoc.id}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          assignedToName: reviewAssignee,
          notes: reviewNotes,
          dueDate: reviewDue || undefined,
        }),
      },
    );
    const data = await res.json();
  ...
  return (
    <div className="-m-8 md:-m-12 h-[calc(100vh-6rem)] flex bg-zinc-950 text-white overflow-hidden">
      ...
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg text-sm font-medium ${toast.type === "ok" ? "bg-emerald-600" : "bg-red-600"}`}
        >
          {toast.type === "ok" ? (
            <Check size={15} />
          ) : (
          ...
          {isSidebarCollapsed ? (
            <div className="flex flex-col items-center gap-4 py-4">
              ...
              <button
                ...
                className={`p-2 rounded-md ${!selectedCategory ? "bg-zinc-800 text-amber-400" : "text-zinc-600"}`}
              >
                <FileText size={18} />
              </button>
            </div>
                ...
                <div className="flex items-center gap-3 truncate">
                  ...
                  <div className="truncate">
                    ...
                    <p className="text-[10px] text-zinc-500 uppercase tracking-wider">
                      {getMimeLabel(selectedDoc.mimeType)} ·{" "}
                ...
                selectedDoc.mimeType === "application/pdf" ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center gap-4 p-6 text-center">
                    ...
                    <a
                      href={selectedDoc.publicUrl}
                      download
                      className="flex items-center gap-2 bg-white text-black px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-zinc-100 transition-colors"
                    >
                ...
                <div className="p-4 flex flex-col gap-4">
                  ...
                  <div className="border-t border-zinc-800 pt-4 flex flex-col gap-2 text-xs text-zinc-500">
                    <div className="flex justify-between">
                      <span>Tipo</span>
                      <span>{getMimeLabel(selectedDoc.mimeType)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Tamaño</span>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>{selectedDoc.uploadedByName || "-"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Fecha</span>
                    ...
                    <div className="flex justify-between">
                      <span>IA</span>
                      <span>
                        {selectedDoc.aiProcessed ? "✓ Procesado" : "Pendiente"}
                      </span>
                    </div>
                  </div>
                </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center justify-between mb-1">
                                <span
                                  className={`font-medium ${colorMap[r.status] || "text-zinc-400"}`}
                                >
                              ...
                              {r.notes && (
                                <p className="text-zinc-400 mt-1">{r.notes}</p>
                              )}
                            </div>
                          );
                        })}
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          {
                            label: "Visitas",
                            value: shares.reduce((a, s) => a + s.viewCount, 0),
                          },
                          {
                            label: "Aperturas",
                            value: shares.reduce(
      ...
      {moveCatModal && (
        <MoveModal
          ...
          onConfirm={(newParentId) =>
            handleMoveCategory(moveCatModal, newParentId)
          }
              ...
              {uploadFile ? (
                ...
                <>
                  ...
                  <p className="text-sm text-zinc-400">
                    ...
                    <span className="text-white underline">selecciona</span>
                  </p>
                  <p className="text-xs text-zinc-600 mt-1">
                    PDF, Word, Excel, PowerPoint, imágenes (máx. 50 MB)
                  </p>
                </>
              )}
              <input
            ...
            {uploading && (
              <div className="flex flex-col gap-2">
                {[
                  ...
                  <div
                    key={s.step}
                    className={`flex items-center gap-2 text-sm ${uploadStep === s.step ? "text-white" : ["uploading", "saving", "ai", "done"].indexOf(uploadStep) > ["uploading", "saving", "ai", "done"].indexOf(s.step) ? "text-emerald-400" : "text-zinc-600"}`}
                  >
                    {uploadStep === s.step ? (
                      <Loader2 size={14} className="animate-spin" />
                    ) : ["uploading", "saving", "ai", "done"].indexOf(
                        uploadStep,
                      ) >
                      ["uploading", "saving", "ai", "done"].indexOf(s.step) ? (
                      <Check size={14} />
                    ) : (
...
function MoveModal({
  ...
  const valid = categories.filter((c) => {
    if (
      ...
      (c.id === excludeId ||
        c.path.startsWith(
          categories.find((x) => x.id === excludeId)?.path + "/",
        ))
    )
      return false;
    return true;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-sm p-6 flex flex-col gap-5">
        ...
        <div className="bg-zinc-800 rounded-xl overflow-hidden max-h-64 overflow-y-auto">
          ...
          {valid.map((c) => (
            ...
            >
              <Folder
                size={13}
                className={c.color ? "" : "text-amber-400"}
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2 rounded-xl text-sm text-zinc-400 hover:bg-zinc-800"
          >
            Cancelar
          </button>
          <button
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-sm p-6 flex flex-col gap-5">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-white">
            {mode === "create"
              ? `Nueva categoría${parent ? ` en "${parent.name}"` : ""}`
