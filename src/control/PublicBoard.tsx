...
export default function PublicBoard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="flex h-14 items-center gap-2.5 border-b border-slate-200 bg-white px-4 sm:px-5">
        ...
        <span className="ml-1 truncate rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-500 sm:ml-2">Solo lectura</span>
      </header>
            ...
            <div className="mb-4 mt-4 flex items-center gap-1 overflow-x-auto border-b border-slate-200">
              {([['grid', 'Tablero', Table2], ['analitica', 'Analítica', BarChart3]] as const).map(([key, label, Icon]) => (
                <button
                  key={key}
                  onClick={() => setTab(key)}
                  className={`-mb-px inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap border-b-2 px-3.5 py-2.5 text-[13px] font-semibold transition ${
