...
import { ecoStartLogin, ecoVerifyLoginCode } from './lib/session'

export function EcosistemaLogin({ onSuccess }: { onSuccess: () => void }) {
  ...
  const submit = async (e?: FormEvent) => {
    ...
    const r = await ecoStartLogin(identifier, password)
    setLoading(false)
  ...
  const verify = async (e: FormEvent) => {
    e.preventDefault()
    setError(''); setInfo(''); setLoading(true)
    const r = await ecoVerifyLoginCode(code)
    setLoading(false)
      ...
      <section className="flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          <div className="mb-6 flex items-center gap-2.5">
            ...
            <div className="text-sm font-black tracking-tight">ECOSISTEMA<span className="text-indigo-600"> DIGITAL</span></div>
          </div>
          ...
          <p className="mb-6 mt-1 text-sm text-slate-500">
            {step === 'credentials' ? 'Accede con tu cuenta.' : `Ingresa el código de 6 dígitos que enviamos a ${maskedEmail || 'tu correo'}.`}
          </p>
          ...
          <div className="mt-6 border-t border-slate-100 pt-4 text-center">
            <Link to="/admin/login" className="inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-slate-400 hover:text-indigo-600">
              <ShieldCheck size={14} /> ¿Eres administrador? Entra al panel
            </Link>
          </div>
        </div>
      </section>
